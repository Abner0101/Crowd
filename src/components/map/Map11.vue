...
export default {
  ...
  data() {
    ...
    // ProGridAdd:判断单元格图层是否已加载,ProGrid:1000米单元格图
    let ProGridObj = { ProGrid: null, ProGridAdd: 0, ProGridDetail: false };
    // blockAdd:判断区块图层是否已加载,block:区块图层
    let blockObj = { block: null, blockAdd: 0 };
    // channelAdd:判断渠道图层是否已加载,channel:渠道图层
    let channelObj = { channel: null, channelAdd: 0, channelDetail: false };
    // distributionOfChannelAdd:判断渠道合理性图层是否已加载,distributionOfChannel:渠道合理性图层
    let distributionOfChannelObj = {
      distributionOfChannel: null,
      distributionOfChannelAdd: 0,
      distributionOfChannelDetail: false
    };
    //respfie:责任田图层
    let respfieObj = {
      respfie: null,
    ...
    //respfieBounder:责任田范围图层
    let respfieBoundObj = {
      respfieBound: null,
      respfieBoundAdd: 0
    }; 
    //综合网格范围图层
    let generalGridBounderObj = {
      generalGridBounder: null,
      generalGridBounderAdd: 0
    }
    //siteRObj:业务选址结果图层，siteRAdd：判断业务选址结果图层是否加载,gridResultDetail：控制渠道单元格选址结果详情页，ProGridResultDetail：控制家宽选址结果详情页
    let siteRObj = { siteR: null, siteRAdd: false, gridResultDetail:false, ProGridResultDetail:false};
    //busCircle:商圈图层,busCircleAdd:判断商圈图层是否加载，busCircleDetail:判断商圈详情页是否显示
    let busCircleObj={
      busCircle:null,
      busCircleAdd:0,
      busCircleDetail:false
    }
    ...
    return {
      ...
      distance: "",
      clickCoor: "",
      gridIdObj: { gridIdLayer: null, gridIdAdd: 0 },
      proGridIdObj: { proGridIdLayer: null, proGridIdAdd: 0 },
      distributionOfChannelObj,
      highLightValidate_obj,
  ...
  methods: {
    ...
    judgeCurrentLayer(
      ...
      if (channelObj.channelAdd) {
        map.removeLayer(channelObj.channel);
        map.addLayer(channelObj.channel);
      }
      if (ProGridObj.ProGridAdd) {
        map.removeLayer(ProGridObj.ProGrid);
        map.addLayer(ProGridObj.ProGrid);
      }
      if (busCircleObj.busCircleAdd) {
        map.removeLayer(busCircleObj.busCircle);
        map.addLayer(busCircleObj.busCircle);
      }
      if (siteRObj.siteRAdd) {
        map.removeLayer(siteRObj.siteR);
        map.addLayer(siteRObj.siteR);
      }
      if (respfieObj.respfieAdd) {
        map.removeLayer(respfieObj.respfie);
        map.addLayer(respfieObj.respfie);
      }
      if (cityObj.cityAdd) {
        map.removeLayer(cityObj.city);
        map.addLayer(cityObj.city);
      }
      if (blockObj.blockAdd) {
        map.removeLayer(blockObj.block);
        map.addLayer(blockObj.block);
      }
      if(gridBoundObj.gridBoundAdd) {
        map.removeLayer(gridBoundObj.gridBound);
        map.addLayer(gridBoundObj.gridBound);
      }
      if(respfieBoundObj.respfieBoundAdd) {
        map.removeLayer(respfieBoundObj.respfieBound);
        map.addLayer(respfieBoundObj.respfieBound);
      }
      if(generalGridBounderObj.generalGridBounderAdd) {
        map.removeLayer(generalGridBounderObj.generalGridBounder);
        map.addLayer(generalGridBounderObj.generalGridBounder);
      }
      if (highLightVector_obj.highLightAdd) {
        map.removeLayer(highLightVector_obj.highLightVector);
        map.addLayer(highLightVector_obj.highLightVector);
      }
      if (highLightRespfieObj.highLightRespfieAdd) {
        map.removeLayer(highLightRespfieObj.highLightRespfie);
        map.addLayer(highLightRespfieObj.highLightRespfie);
      }
      if (highLightBusCircleObj.highLightBusCircleAdd) {
        map.removeLayer(highLightBusCircleObj.highLightBusCircle);
        map.addLayer(highLightBusCircleObj.highLightBusCircle);
      }
    ...
    //控制选址业务结果图层的加载
    changeresult_(){
      this.siteRObj.siteRAdd =!this.siteRObj.siteRAdd ;//记录是否已经添加了当前的单元格选址的结果图层
      this.siteRObj.gridResultDetail=!this.siteRObj.gridResultDetail;
      this.siteRObj.ProGridResultDetail=!this.siteRObj.ProGridResultDetail;
      if(this.siteRObj.siteRAdd == false){
        this.map.removeLayer(this.siteRObj.siteR);
      }
      else if(this.siteRObj.siteRAdd == true){
        this.map.addLayer(this.siteRObj.siteR);
      }
    },
    //根据fid对众包图层的要素进行删除
    ...
    addModifyLayer(){
      ...
      let featureRequest = new ol.format.WFS().writeGetFeature({
        ...
        // filter: ol.format.filter.like('school_id', fid)
      })
  ...
  created() {
      ...
      switch (data) {
        case "百度":
          ...
          this.judgeCurrentLayer(
            ...
            this.blockObj,
            this.gridBoundObj,
            this.respfieBoundObj,
            this.crowdDrawObject,
            this.generalGridBounderObj,
            this.channelObj,
            this.ProGridObj,
            this.busCircleObj,
            this.siteRObj,
            this.respfieObj,
            this.highLightVector_obj,
            this.highLightRespfieObj,
            this.highLightBusCircleObj,
    ...
    Bus.$on("changeGrid", data => {
      ...
      // 移除已加载的单元格高亮图层
      if (this.highLightVector_obj.highLightAdd === 1) {
        this.map.removeLayer(this.highLightVector_obj.highLightVector);
        this.highLightVector_obj.highLightAdd = 0;
      }
      // 移除已加载的渠道一千米范围图层
      if (this.circleVectorObj.circleVector !== null) {
        this.map.removeLayer(this.circleVectorObj.circleVector);
        this.circleVectorObj.circleVector = null;
      }
      // 移除渠道合理性高亮图层
      if (this.highLightValidate_obj.highLightValidateAdd === 1) {
        this.map.removeLayer(this.highLightValidate_obj.highLightValidate);
        this.highLightValidate_obj.highLightValidateAdd = 0;
      }
      // 移除责任田高亮图层
      if(this.highLightRespfieObj.highLightRespfieAdd) {
        this.map.removeLayer(this.highLightRespfieObj.highLightRespfie);
        this.highLightRespfieObj.highLightRespfieAdd = 0;
      }
      //移除商圈高亮图层
      if(this.highLightBusCircleObj.highLightBusCircleAdd) {
        this.map.removeLayer(this.highLightBusCircleObj.highLightBusCircle);
        this.highLightBusCircleObj.highLightBusCircleAdd = 0;
      }
      // 移除所有图层
      ...
      this.map.removeLayer(this.blockObj.block);
      this.blockObj.blockAdd = 0;
      this.map.removeLayer(this.channelObj.channel);
      this.map.removeLayer(this.gridBoundObj.gridBound)
      this.gridBoundObj.gridBoundAdd = 0;
      this.channelObj.channelAdd = 0;
      this.map.removeLayer(this.ProGridObj.ProGrid);
      this.ProGridObj.ProGridAdd = 0;
      this.map.removeLayer(this.gridIdObj.gridIdLayer);
      this.gridIdObj.gridIdAdd = 0;
      this.map.removeLayer(this.proGridIdObj.proGridIdLayer);
      this.proGridIdObj.proGridIdAdd = 0;
      this.map.removeLayer(this.distributionOfChannelObj.distributionOfChannel);
      this.distributionOfChannelObj.distributionOfChannelAdd = 0;
      this.map.removeLayer(this.respfieObj.respfie);
      this.respfieObj.respfieAdd = 0;
      this.map.removeLayer(this.respfieBoundObj.respfieBound);
      this.respfieBoundObj.respfieBoundAdd = 0;
      this.map.removeLayer(this.generalGridBounderObj.generalGridBounder);
      this.generalGridBounderObj.generalGridBounderAdd = 0;
      this.map.removeLayer(this.busCircleObj.busCircle);
      this.busCircleObj.busCircleAdd = 0;
      ...
      for (let i in data) {
        switch (data[i]) {
          ...
          case "区块":
            if (window.currentCity == "广东省") {
              this._updateFilterByCity(this.blockObj.block, "");
            } else {
              this._updateFilterByCity(this.blockObj.block, window.currentCity);
            }
            this.map.addLayer(this.blockObj.block);
            this.blockObj.block.set("opacity", 1);
            this.blockObj.blockAdd = 1;
            break;
          case "单元格":
            if (window.currentCity == "广东省") {
              this._updateFilterByCity(this.gridBoundObj.gridBound, "");
            } else {
              this._updateFilterByCity(this.gridBoundObj.gridBound, window.currentCity);
            }
            this.map.addLayer(this.gridBoundObj.gridBound);
            this.gridBoundObj.gridBound.set("opacity", 1);
            this.gridBoundObj.gridBoundAdd = 1;
            break;
          case "渠道点得分":
            if (window.currentCity == "广东省") {
              this._updateFilterByChannelCity(this.channelObj.channel, "");
            } else {
              this._updateFilterByChannelCity(
                this.channelObj.channel,
                window.currentCity
              );
            }
            this.map.addLayer(this.channelObj.channel);
            this.channelObj.channel.set("opacity", 1);
            this.channelObj.channelAdd = 1;
            break;
          case "渠道单元格选址评分":
             if(this.siteRObj.siteR){
            this.map.removeLayer(this.siteRObj.siteR);
            }
            this.gridObj.grid = this._sourceLayer(
              this.geoserverProperties.url,
              this.geoserverProperties.layerName03,
              this.geoserverProperties.style03_1
            );
            if (window.currentCity == "广东省") {
              this._updateFilterByCity(this.gridObj.grid, "");
            } else {
              this._updateFilterByCity(this.gridObj.grid, window.currentCity);
            }
            this.map.addLayer(this.gridObj.grid);
            if(this.siteRObj.siteR && this.siteRObj.siteRAdd == true){
              this.map.addLayer(this.siteRObj.siteR);
            }
            this.gridObj.grid.set("opacity", 1);
            this.gridObj.gridAdd = 1;
            Bus.$emit("legendButtonShow", true);
            break;
          case "渠道单元格id":
            this.gridIdObj.gridIdLayer = this._sourceLayer(
              this.geoserverProperties.url,
              this.geoserverProperties.layerName03,
              this.geoserverProperties.style03_4
            );
            if (window.currentCity == "广东省") {
              this._updateFilterByCity(this.gridIdObj.gridIdLayer, "");
            } else {
              this._updateFilterByCity(
                this.gridIdObj.gridIdLayer,
                window.currentCity
              );
            }
            this.map.addLayer(this.gridIdObj.gridIdLayer);
            this.gridIdObj.gridIdLayer.set("opacity", 1);
            this.gridIdObj.gridIdAdd = 1;
            Bus.$emit("legendButtonShow", true);
            break;
          case "家宽服务站单元格选址评分":
            if(this.siteRObj.siteR){
            this.map.removeLayer(this.siteRObj.siteR);
              ...
              this.ProGridObj.ProGrid = this._sourceLayer(
              this.geoserverProperties.url,
              this.geoserverProperties.layerName04,
              this.geoserverProperties.style04_1
            );
            if (window.currentCity == "广东省") {
              this._updateFilterByCity(this.ProGridObj.ProGrid, "");
            } else {
              this._updateFilterByCity(
                this.ProGridObj.ProGrid,
                window.currentCity
              );
            }
            this.map.addLayer(this.ProGridObj.ProGrid);
            if(this.siteRObj.siteR && this.siteRObj.siteRAdd == true){
              this.map.addLayer(this.siteRObj.siteR);
            }
            this.ProGridObj.ProGrid.set("opacity", 1);
            this.ProGridObj.ProGridAdd = 1;
            Bus.$emit("legendButtonShow", true);
            break;
          case "家宽单元格id":
            if (window.currentCity == "广东省") {
              this._updateFilterByCity(
                this.proGridIdObj.proGridIdLayer,
                this.geoserverProperties.style04_1
              );
            } else {
              this._updateFilterByCity(
            ...
            this.map.addLayer(this.proGridIdObj.proGridIdLayer);
            this.proGridIdObj.proGridIdLayer.set("opacity", 1);
            this.proGridIdObj.proGridIdAdd = 1;
            Bus.$emit("legendButtonShow", true);
            break;
          case "渠道合理性分布":
            if (window.currentCity == "广东省") {
              this._updateFilterByCity(
                this.distributionOfChannelObj.distributionOfChannel,
                ""
              );
            } else {
              this._updateFilterByCity(
            ...
            this.distributionOfChannelObj.distributionOfChannel.set(
              "opacity",
              1
            );
            this.distributionOfChannelObj.distributionOfChannelAdd = 1;
            Bus.$emit("legendButtonShow", true);
            break;
          case "责任田范围":
            if (window.currentCity == "广东省") {
              this._updateFilterByCity(
                this.respfieBoundObj.respfieBound,
                ""
              )
            }else {
              this._updateFilterByCity(
            ...
            this.respfieBoundObj.respfieBound.set(
              "opacity",
              1
            );
            this.respfieBoundObj.respfieBoundAdd = 1;
            Bus.$emit("legendButtonShow", true);
            break;
          case "综合网格范围":
            if (window.currentCity == "广东省") {
              this._updateFilterByCity(
                this.generalGridBounderObj.generalGridBounder,
                ""
              );
            } else {
              this._updateFilterByCity(
            ...
            this.generalGridBounderObj.generalGridBounder.set(
              "opacity",
              1
            );
            this.generalGridBounderObj.generalGridBounderAdd = 1;
            Bus.$emit("legendButtonShow", true);
            break;
          case "责任田渠道合理性分布":
            if (window.currentCity == "广东省") {
              this._updateFilterByCity(
                this.respfieObj.respfie,
                ""
              );
            } else {
              this._updateFilterByCity(
            ...
            this.respfieObj.respfie.set(
              "opacity",
              1
            );
            this.respfieObj.respfieAdd = 1;
            Bus.$emit("legendButtonShow", true);
            break;
          case "商圈":
            if (window.currentCity == "广东省") {
              this._updateFilterByCity(
                this.busCircleObj.busCircle,
                ""
              );
            } else {
              this._updateFilterByCity(
            ...
            this.busCircleObj.busCircle.set(
              "opacity",
              1
            );
            this.busCircleObj.busCircleAdd = 1;
            Bus.$emit("legendButtonShow", true);
            break;
          case "众包图层":
    ...
    // 显示图层详细信息
    Bus.$on("showInfo", data => {
      Bus.$emit("rateLabelShow", false); // 用于显示家宽占有率的组件不显示
      Bus.$emit("closeLabel"); // 关闭渠道所属单元格显示卡片
      switch (data) {
        case "grid":
          this.gridObj.gridDetail = true;
          this.ProGridObj.ProGridDetail = false;
          this.channelObj.channelDetail = false;
          this.respfieObj.respfieDetail = false;
          this.busCircleObj.busCircleDetail=false;
          this.distributionOfChannelObj.distributionOfChannelDetail = false;
          break;
        case "channel":
          this.channelObj.channelDetail = true;
          this.gridObj.gridDetail = false;
          this.ProGridObj.ProGridDetail = false;
          this.respfieObj.respfieDetail = false;
          this.busCircleObj.busCircleDetail=false;
          this.distributionOfChannelObj.distributionOfChannelDetail = false;
          break;
        case "ProGrid":
          this.ProGridObj.ProGridDetail = true;
          this.channelObj.channelDetail = false;
          this.gridObj.gridDetail = false;
          this.respfieObj.respfieDetail = false;
          this.busCircleObj.busCircleDetail=false;
          this.distributionOfChannelObj.distributionOfChannelAdd = false;
          break;
        case "respfie":
          this.respfieObj.respfieDetail = true;
          this.ProGridObj.ProGridDetail = false;
          this.channelObj.channelDetail = false;
          this.gridObj.gridDetail = false;
          this.busCircleObj.busCircleDetail=false;
          ...
          break;
        case "businessCircle":
          this.busCircleObj.busCircleDetail=true;
          this.respfieObj.respfieDetail = false;
          this.ProGridObj.ProGridDetail = false;
          this.channelObj.channelDetail = false;
          this.gridObj.gridDetail = false;
        ...
        case "channelValidate":
          this.distributionOfChannelObj.distributionOfChannelDetail = true;
          this.busCircleObj.busCircleDetail=false;
          this.respfieObj.respfieDetail = false;
          this.ProGridObj.ProGridDetail = false;
          this.channelObj.channelDetail = false;
      ...
      if (this.highLightVector_obj.highLightVector !== null) {
        this.map.removeLayer(this.highLightVector_obj.highLightVector);
      }
      if (this.circleVectorObj.circleVector !== null) {
        this.map.removeLayer(this.circleVectorObj.circleVector);
  ...
  mounted() {
    ...
    // 广东省渠道单元格得分图层分级样式
    this.geoserverProperties.style03_1 = this.initialData.geoserver.style03_1;
    // 广东省渠道单元格得分top20渲染风格图层分级样式
    this.geoserverProperties.style03_2 = this.initialData.geoserver.style03_2;
    // 广东省1km单元格高亮显示图层样式
    this.geoserverProperties.style03_3 = this.initialData.geoserver.style03_3;
    // 广东省渠道单元格id样式
    this.geoserverProperties.style03_4 = this.initialData.geoserver.style03_4;
    // 广东省家宽单元格得分图层分级样式
    this.geoserverProperties.style04_1 = this.initialData.geoserver.style04_1;
    // 广东省家宽单元格id样式
    this.geoserverProperties.style04_2 = this.initialData.geoserver.style04_2;
    // 广东省渠道点得分图层样式
    this.geoserverProperties.style05 = this.initialData.geoserver.style05;
    // 广东省渠道合理性得分图层分级样式
    this.geoserverProperties.style06_1 = this.initialData.geoserver.style06_1;
    // 广东省渠道合理性得分图层选择样式
    this.geoserverProperties.style06_2 = this.initialData.geoserver.style06_2;
    //广深东佛责任田范围图层样式
    this.geoserverProperties.style07_1 = this.initialData.geoserver.style07_1;
    //广深东佛责任田得分图层分级样式
    this.geoserverProperties.style08_1 = this.initialData.geoserver.style08_1;
    //广深东佛责任田得分图层选择样式
    this.geoserverProperties.style08_2 = this.initialData.geoserver.style08_2;
    //广东省综合网格图层样式
    this.geoserverProperties.style09_1 = this.initialData.geoserver.style09_1;
    //广东省商圈图层样式
    this.geoserverProperties.style10_1 = this.initialData.geoserver.style10_1;
    //广东省商圈图层选择样式
    this.geoserverProperties.style10_2 = this.initialData.geoserver.style10_2;
    //广东省单元格范围图层高亮样式
    this.geoserverProperties.style12 = this.initialData.geoserver.style12;
    ...
    // 区块
    this.blockObj.block = this._sourceLayer(
      this.geoserverProperties.url,
      this.geoserverProperties.layerName02,
      this.geoserverProperties.style02
    );
    // 渠道点
    this.channelObj.channel = this._sourceLayer(
      this.geoserverProperties.url,
      this.geoserverProperties.layerName05,
      this.geoserverProperties.style05
    );
    // 渠道单元格
    this.gridObj.grid = this._sourceLayer(
      this.geoserverProperties.url,
    ...
    // 家宽单元格id
    this.proGridIdObj.proGridIdLayer = this._sourceLayer(
      this.geoserverProperties.url,
      this.geoserverProperties.layerName04,
      this.geoserverProperties.style03_4
    );
    // 渠道合理性分布图层
    this.distributionOfChannelObj.distributionOfChannel = this._sourceLayer(
      this.geoserverProperties.url,
      this.geoserverProperties.layerName06,
      ""
    );
    // 责任田边界图层
    this.respfieBoundObj.respfieBound = this._sourceLayer(
      this.geoserverProperties.url,
      this.geoserverProperties.layerName07,
      this.geoserverProperties.style07_1
    );
    // 责任田图层
    this.respfieObj.respfie = this._sourceLayer(
      this.geoserverProperties.url,
      this.geoserverProperties.layerName08,
      this.geoserverProperties.style08_1
    );
    //综合网格边界图层
    this.generalGridBounderObj.generalGridBounder = this._sourceLayer(
      this.geoserverProperties.url,
      this.geoserverProperties.layerName09,
      this.geoserverProperties.style09_1
    );
    //商圈图层
    this.busCircleObj.busCircle=this._sourceLayer(
      this.geoserverProperties.url,
      this.geoserverProperties.layerName10,
      this.geoserverProperties.style10_1
    )
    // 单元格高亮图层
    this.highLightVector_obj.highLightVector = this._sourceLayer(
      this.geoserverProperties.url,
      this.geoserverProperties.layerName03,
      this.geoserverProperties.style03_3
    );
    // 渠道合理性分布高亮图层
    this.highLightValidate_obj.highLightValidate = this._sourceLayer(
      this.geoserverProperties.url,
      this.geoserverProperties.layerName06,
      this.geoserverProperties.style06_2
    );
    // 责任田高亮图层
    this.highLightRespfieObj.highLightRespfie = this._sourceLayer(this.geoserverProperties.url, this.geoserverProperties.layerName08, this.geoserverProperties.style08_2);
    //商圈高亮图层
    this.highLightBusCircleObj.highLightBusCircle = this._sourceLayer(this.geoserverProperties.url, this.geoserverProperties.layerName10, this.geoserverProperties.style10_2)
     //单元格图层
    this.gridBoundObj.gridBound = this._sourceLayer(
      this.geoserverProperties.url,
      this.geoserverProperties.layerName12,
      this.geoserverProperties.style12
    );
    //众包图层
    ...
    const overviewMapControl = new ol.control.OverviewMap({
      ...
      layers: [this.cityObj.city], // 鹰眼中加载同坐标系下不同数据源的图层
    ...
    this.map.on("singleclick", evt => {
      // 商业报告选点画圆
      if (this.drawArea && this.selectPoint) {
        this.clickCoor = evt.coordinate;
        Bus.$emit("getPosition_report", evt.coordinate);
        if (this.circleVectorObj.circleVector !== null) {
          this.map.removeLayer(this.circleVectorObj.circleVector);
          this.circleVector = null;
        }
        this._addCircle(this.map, this.circleVectorObj, 1, [evt.coordinate[0], evt.coordinate[1]], this.distance);
        this.selectPoint = false;
        return;
      } else if (this.drawArea && !this.selectPoint) {
        return;
      }
      // 坐标拾取
      ...
      //删除指定要素和查看点击要素的字段
      if(!this.drawArea & !this.drawPolygonObject.drawFlag & !this.siteRObj.siteRAdd & this.crowdDrawObject.crowdLayerAdd & this.deleteFlag || this.propertiesFlag||this.modifyFlag){
        if(this.deleteFlag){
            ...
            axios.get(url).then(response => {
              if(response.data.features.length > 0){
                this.$confirm('是否删除该要素？', '提示', {
                  ...
                  //取得fid中的数值
                  let fid = feature.id.substring(feature.id.lastIndexOf(".")+1);
                  this.deleteFeatureByFid(fid);
                })
              }
            })
